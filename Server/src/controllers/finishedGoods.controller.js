...
export const getOne = async (req, res, next) => {
    try {
        ...
        if (!good.rows[0]) return res.status(404).json({ error: 'Finished good not found' });
...
export const create = async (req, res, next) => {
    try {
        const { name, unit, price_per_unit, category_id, expiry_duration_days } = req.body;
        ...
        const result = await pool.query(
            ...
            [name, unit || 'pieces', price_per_unit || 0, category_id || null, expiry_duration_days || null]
        );
...
export const update = async (req, res, next) => {
    try {
        ...
        const result = await pool.query(
            ...
            [name || null, unit || null, price_per_unit ?? null,
             category_id || null, expiry_duration_days || null, req.params.id]
        );
        if (!result.rows[0]) return res.status(404).json({ error: 'Finished good not found' });
...
export const remove = async (req, res, next) => {
    try {
        ...
        if (!check.rows[0]) return res.status(404).json({ error: 'Finished good not found' });

        if (check.rows[0].quantity_in_stock > 0) {
            return res.status(409).json({
                error: 'Cannot delete a product with stock remaining. Dispatch or adjust stock first.'
            });
        }
