...
export const getAll = async (req, res, next) => {
    try {
        const { item_type, movement_type, item_id, from, to, limit = 100, offset = 0 } = req.query;
...
export const getOne = async (req, res, next) => {
    try {
        ...
        if (!result.rows[0]) return res.status(404).json({ error: 'Movement not found' });
    ...
    try {
        const result = await pool.query(
            `SELECT
             ...
             LEFT JOIN raw_materials rm ON sm.item_type = 'raw_material' AND rm.id = sm.item_id
             LEFT JOIN finished_goods fg ON sm.item_type = 'finished_good' AND fg.id = sm.item_id
             GROUP BY sm.item_type, sm.item_id, item_name, unit
