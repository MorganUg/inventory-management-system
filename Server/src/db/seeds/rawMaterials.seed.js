export const seedRawMaterials = async (pool) => {
    ...
    const cats = await pool.query('SELECT id, name FROM categories WHERE type = $1', ['raw_material']);
    ...
    for (const m of materials) {
        await pool.query(
            ...
            [
                m.name, m.unit, m.quantity_in_stock, m.reorder_level,
                m.cost_per_unit,
                catMap[m.category] || null,
                supMap[m.supplier] || null
            ]
        );
        console.log(`${m.name} saved`);
    }
};