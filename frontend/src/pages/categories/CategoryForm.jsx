...
export default function CategoryForm({ initial, onSuccess }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // error handled above via mutation
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <select
          ...
          disabled={!!initial}
          className={`mt-1 w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 disabled:opacity-50 disabled:cursor-not-allowed ${errors.type ? "border-red-400 bg-red-50" : "border-gray-300"}`}
        >
