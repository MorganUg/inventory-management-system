...
export default function RestocksPage() {
  ...
  const totalSpend = restocks.reduce((sum, r) => sum + parseFloat(r.total_cost || 0), 0);
  const uniqueMaterials = new Set(restocks.map(r => r.material_id)).size;
  const todayRestocks = restocks.filter(r => 
    new Date(r.received_at).toDateString() === new Data().toDateString()
  ).length;
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              {['Material', 'Supplier', 'Qty Recieved', 'Cost/Unit', 'Total Cost', 'Received By', 'Date', 'Notes'].map(h => (
