...
export const useProductionReport = (filters = {}) =>
  useQuery({
    ...
    queryFn: () => api.get('/reports/production', { params: buildParams(filters) }).then((res) => res.data),
  });
...
export const useDispatchReport = (filters = {}) =>
  useQuery({
    ...
    queryFn: () => api.get('/reports/dispatches', { params: buildParams(filters) }).then((res) => res.data),
  });

export const useConsumptionReport = (filters = {}) =>
  useQuery({
    queryKey: ['reports', 'consumption', filters],
    queryFn: () => api.get('/reports/consumption', { params: buildParams(filters) }).then((res) => res.data),
  });

export const useProductionChart = () => 
  useQuery({
    queryKey: ['reports', 'charts', 'production'],
    queryFn: () => api.get('/reports/charts/production').then((res) => res.data),
  });

export const useDispatchChart = () => 
  useQuery({
    queryKey: ['reports', 'charts', 'dispatches'],
    queryFn: () => api.get('/reports/charts/dispatches').then((res) => res.data),
  });

export const useTopProductsChart = () =>
  useQuery({
    queryKey: ['reports', 'charts', 'top-products'],
    queryFn: () => api.get('/reports/charts/top-products').then((res) => res.data),
  });

export const useStockLevelsChart = () => 
  useQuery({
    queryKey: ['reports', 'charts', 'stock-levels'],
    queryFn: () => api.get('/reports/charts/stock-levels').then((res) => res.data),
  });